...
/// Returns the 16-byte bitmap for the given ASCII character.
/// Out-of-range characters return a filled box (missing glyph indicator).
pub fn glyph(ch: u8) -> &'static [u8; 16] {
    if ch >= FONT_FIRST && ch <= FONT_LAST {
        &FONT_DATA[(ch - FONT_FIRST) as usize]
    } else {
        &MISSING_GLYPH
    }
}
